...
pub static TABLE_ROW: Lazy<Selector> =
    Lazy::new(|| Selector::parse("tr").expect("テーブル行セレクタのパースに失敗"));

pub static TABLE_CELL: Lazy<Selector> =
    Lazy::new(|| Selector::parse("td").expect("テーブルセルセレクタのパースに失敗"));

pub static CARD_IMAGE: Lazy<Selector> = Lazy::new(|| {
    Selector::parse(".cardDetail__img img").expect("カード画像セレクタのパースに失敗")
});

pub static CARD_LINK: Lazy<Selector> =
    Lazy::new(|| Selector::parse("a.c-box").expect("カードリンクセレクタのパースに失敗"));
...
pub static LIMIT_PATTERN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(\d+)").expect("リミットパターンの正規表現コンパイルに失敗"));

pub static LEVEL_PATTERN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"lv(\d+)").expect("レベルパターンの正規表現コンパイルに失敗"));

pub static POWER_PATTERN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(\d+)").expect("パワーパターンの正規表現コンパイルに失敗"));

pub static COIN_PATTERN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(\d+)").expect("コインパターンの正規表現コンパイルに失敗"));

pub static USE_PATTERN: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"【(.*?)】").expect("使用タイミングパターンの正規表現コンパイルに失敗")
});

pub static PHASE_PATTERN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"【(.*?)】").expect("フェーズパターンの正規表現コンパイルに失敗"));

pub static CLASS_PATTERN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"〈([^〉]+)〉").expect("クラスパターンの正規表現コンパイルに失敗"));

pub static SELECTOR_COST_ITEM: Lazy<Selector> =
    Lazy::new(|| Selector::parse("span.costItem").expect("コストアイテムセレクタのパースに失敗"));

pub static SELECTOR_COST_SINGLE: Lazy<Selector> =
    Lazy::new(|| Selector::parse("span.costSingle").expect("単一コストセレクタのパースに失敗"));

pub static SELECTOR_COST_COLOR: Lazy<Selector> =
    Lazy::new(|| Selector::parse("span.costColor").expect("色コストセレクタのパースに失敗"));

pub static SELECTOR_COST_IMG: Lazy<Selector> =
    Lazy::new(|| Selector::parse("span.costImg img").expect("コスト画像セレクタのパースに失敗"));

pub static SELECTOR_ALT: Lazy<Selector> =
    Lazy::new(|| Selector::parse("img").expect("画像セレクタのパースに失敗"));
