...
pub use encoder::{encode_skill_text, encode_burst_text, encode_multiple_texts};
pub use decoder::{decode_skill_text, decode_burst_text, get_original_pattern, get_available_icons, TextSegment};
...
mod integration_tests {
    ...
    fn test_all_defined_icons_work() {
        for rule in ICON_RULES {
            ...
            assert_eq!(encoded, expected_encoded, "Encoding failed for: {}", rule.pattern);
            
            // デコードテスト
            let segments = decode_skill_text(&encoded);
            assert!(segments.len() >= 3, "Expected at least 3 segments for: {}", rule.pattern);
            
            // 中央のセグメントがアイコンであることを確認
            match &segments[1] {
                TextSegment::Icon { component, .. } => {
                    assert_eq!(component, rule.component, "Component mismatch for: {}", rule.pattern);
                },
