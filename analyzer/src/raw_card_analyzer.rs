...
impl std::fmt::Display for AnalysisError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "Analysis error for card {}: {}", self.card_id, self.message)
    }
}
...
pub trait RawCardAnalyzer {
    ...
    async fn analyze_batch(&self, raw_cards: &[RawCardDb]) -> Vec<Result<CreateCard, AnalysisError>> {
...
impl RawCardAnalyzer for DefaultRawCardAnalyzer {
    async fn analyze(&self, raw_card: &RawCardDb) -> Result<CreateCard, AnalysisError> {
        ...
        let (feature_bits1, feature_bits2) = self.analyze_features(
            &raw_card.skill_text,
            &raw_card.life_burst_text
        );
        ...
        Ok(CreateCard {
            ...
            power: None,
            has_burst: if raw_card.life_burst_text.is_empty() { 2 } else { 1 }, // 1=has burst, 2=no burst
            cost: None,
            level: None,
            limit: None,
            limit_ex: None,
