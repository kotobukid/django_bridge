...
fn test_replacement_patterns() {
    ...
    for pattern in &replace_patterns {
        if pattern.pattern_r.is_match(test_text1) {
            println!(
                "Pattern '{}' matched for self-assassin text",
                pattern.pattern
            );
    ...
    assert!(
        found_self_assassin,
        "Self-assassin pattern should match and replace"
    );
    ...
    for pattern in &replace_patterns {
        if pattern.pattern_r.is_match(test_text2) {
            println!(
                "\nPattern '{}' matched for assassin ability text",
                pattern.pattern
            );
            println!("Replace to: '{}'", pattern.replace_to);
    ...
    assert!(
        found_assassin,
        "Assassin ability pattern should match and replace"
    );
    ...
    fn apply_replacements(text: &str, patterns: &[feature::ReplacePattern]) -> String {
        ...
        for pattern in patterns {
            if pattern.pattern_r.is_match(&result) {
                result = pattern
                    .pattern_r
                    .replace_all(&result, pattern.replace_to)
                    .to_string();
            }
        }
        result
