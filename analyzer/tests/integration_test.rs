...
fn test_migration_compatibility() {
    ...
    assert!(!migrated_features.is_empty(), "何らかのフィーチャーが検出される");
    
    println!("移行されたアナライザーで検出されたフィーチャー: {:?}", migrated_features);
}
...
fn test_parallel_performance() {
    ...
    assert!(!features.is_empty(), "並列処理で複数のフィーチャーが検出される");
    assert!(features.len() >= 3, "少なくとも3つのフィーチャーが検出される");
...
fn test_enhanced_analyzer_features() {
    ...
    // 結果が一致することを確認
    assert_eq!(normal_features, parallel_features, "通常と並列処理の結果が一致");
    assert_eq!(normal_features, preprocessing_features, "通常と前処理付きの結果が一致");
