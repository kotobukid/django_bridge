...
mod regex_tests {
    ...
    fn test_individual_digit_matching() {
        ...
        assert!(
            !ascii_digit_re.is_match("２"),
            "[0-9] should NOT match full-width ２"
        );

        // Test half-width digit
        assert!(digit_re.is_match("2"), "\\d should match half-width 2");
        assert!(
            ascii_digit_re.is_match("2"),
            "[0-9] should match half-width 2"
        );
    }
    ...
    fn test_feature_detection_with_fullwidth() {
        ...
        assert!(
            pattern.is_match(fullwidth_text),
            "Pattern DOES match full-width text because \\d matches full-width digits"
        );
        assert!(
            pattern.is_match(halfwidth_text),
            "Pattern should match half-width text"
        );
    }
    ...
    fn test_to_half_conversion() {
        ...
        assert!(
            pattern.is_match(original),
            "Pattern DOES match original full-width text"
        );
        assert!(
            pattern.is_match(&converted),
            "Pattern should match converted half-width text"
        );
    }
}
