use leptos::prelude::*;

#[component]
pub fn SvgDefinition() -> impl IntoView {
    view! {
        <svg style="position: absolute; width: 0; height: 0; pointer-events: none;" aria-hidden="true">
            <defs>
              <linearGradient gradientUnits="userSpaceOnUse" x1="67.336" y1="-39.914" x2="67.336" y2="-20.406" id="gradient-0" gradientTransform="matrix(0.879611, -0.475694, 0.427208, 0.789956, 21.13001, 25.106044)">
                <stop offset="0" style="stop-color: rgb(164, 255, 219);"></stop>
                <stop offset="1" style="stop-color: rgb(0, 118, 220);"></stop>
              </linearGradient>
              <linearGradient gradientUnits="userSpaceOnUse" x1="39.328" y1="-36.86" x2="39.328" y2="-16.939" id="gradient-1" gradientTransform="matrix(-0.019553, -0.999808, 0.870125, -0.017016, 63.910962, 7.260032)">
                <stop offset="0" style="stop-color: rgb(100% 93.333% 81.961%)"></stop>
                <stop offset="0.155" style="stop-color: rgb(244, 200, 132);"></stop>
                <stop offset="0.354" style="stop-color: rgb(255, 241, 218);"></stop>
                <stop offset="0.513" style="stop-color: rgb(236, 200, 142);"></stop>
                <stop offset="1" style="stop-color: rgb(58.524% 52.764% 42.49%)"></stop>
              </linearGradient>
              <linearGradient gradientUnits="userSpaceOnUse" x1="-34.308" y1="2.922" x2="-34.308" y2="23.677" id="gradient-2" gradientTransform="matrix(0.797531, 0, 0, 0.797531, -6.946306, 4.793859)">
                <stop offset="0" style="stop-color: rgb(255, 216, 0);"></stop>
                <stop offset="1" style="stop-color: rgb(255, 0, 0);"></stop>
              </linearGradient>
              <linearGradient gradientUnits="userSpaceOnUse" x1="246.092" y1="-76.023" x2="246.092" y2="-58.235" id="gradient-3" gradientTransform="matrix(0.838383, -0.545081, 0.494267, 0.760228, -74.772134, 154.221935)">
                <stop offset="0" style="stop-color: rgb(133, 254, 0);"></stop>
                <stop offset="0.195" style="stop-color: rgb(50, 183, 16);"></stop>
                <stop offset="1" style="stop-color: rgb(0, 118, 0);"></stop>
              </linearGradient>
              <radialGradient gradientUnits="userSpaceOnUse" cx="64.965" cy="-139.07" r="10.765" id="gradient-4" gradientTransform="matrix(0.879605, 0.553818, -0.532808, 0.846236, -31.757743, -28.728703)">
                <stop offset="0" style="stop-color: rgb(84.706% 84.706% 84.706%)"></stop>
                <stop offset="1" style="stop-color: rgb(46, 0, 81);"></stop>
              </radialGradient>
              <linearGradient gradientUnits="userSpaceOnUse" x1="91.901" y1="-148.32" x2="91.901" y2="-126.88" id="gradient-5" gradientTransform="matrix(0.774343, -0.632766, 0.271638, 0.332415, 66.401125, -6.197643)">
                <stop offset="0" style="stop-color: rgb(255, 255, 255);"></stop>
                <stop offset="1" style=""></stop>
              </linearGradient>
              <radialGradient gradientUnits="userSpaceOnUse" cx="64.965" cy="-139.07" r="10.765" id="gradient-6" gradientTransform="matrix(0.879605, 0.553818, -0.532808, 0.846236, -3.467001, 50.948943)">
                <stop offset="0" style="stop-color: rgb(84.706% 84.706% 84.706%)"></stop>
                <stop offset="1" style="stop-color: rgb(46, 0, 81);"></stop>
              </radialGradient>
              <linearGradient gradientUnits="userSpaceOnUse" x1="91.901" y1="-148.32" x2="91.901" y2="-126.88" id="gradient-7" gradientTransform="matrix(0.715416, -0.698698, 0.544494, 0.557521, 137.432819, 110.513827)">
                <stop offset="0" style="stop-color: rgb(255, 255, 255);"></stop>
                <stop offset="0.511" style="stop-color: rgb(49, 49, 49);"></stop>
                <stop offset="1" style="stop-color: rgb(255, 255, 255);"></stop>
              </linearGradient>
              <linearGradient gradientUnits="userSpaceOnUse" x1="7.163" y1="46.135" x2="7.163" y2="58.951" id="gradient-8" gradientTransform="matrix(-0.127014, -0.991902, 0.532222, -0.068152, -19.847133, 61.993992)">
                <stop offset="0" style="stop-color: rgb(255, 255, 255);"></stop>
                <stop offset="1" style="stop-color: rgb(49, 15, 15);"></stop>
              </linearGradient>
              <linearGradient gradientUnits="userSpaceOnUse" x1="7.535" y1="40.74" x2="7.535" y2="53.556" id="gradient-9" gradientTransform="matrix(-0.715897, -0.698206, 0.802889, -0.823235, -25.350924, 96.601573)">
                <stop offset="0" style="stop-color: rgb(84.706% 84.706% 84.706%)"></stop>
                <stop offset="0.51" style="stop-color: rgb(108, 103, 103);"></stop>
                <stop offset="1" style="stop-color: rgb(48.42% 48.42% 48.42%)"></stop>
              </linearGradient>

              <symbol id="num0" viewBox="0 0 32 32">
                <path d="M 22.835 29.988 Q 18.887 29.988 16.829 27.538 Q 14.771 25.088 14.771 20.776 Q 14.771 16.436 16.829 13.972 Q 18.887 11.508 22.835 11.508 Q 26.755 11.508 28.827 13.972 Q 30.899 16.436 30.899 20.776 Q 30.899 25.088 28.827 27.538 Q 26.755 29.988 22.835 29.988 Z M 22.835 26.292 Q 24.571 26.292 25.341 24.766 Q 26.111 23.24 26.111 20.776 Q 26.111 18.312 25.341 16.828 Q 24.571 15.344 22.835 15.344 Q 21.127 15.344 20.343 16.828 Q 19.559 18.312 19.559 20.776 Q 19.559 23.24 20.343 24.766 Q 21.127 26.292 22.835 26.292 Z" style="fill: rgb(255, 255, 255); paint-order: fill; stroke: rgb(0, 0, 0);"/>
              </symbol>

              <symbol id="num1" viewBox="0 0 32 32">
                <path d="M 22.35 29.543 L 22.35 16.635 L 19.41 18.231 L 17.506 14.423 L 23.19 11.763 L 27.082 11.763 L 27.082 29.543 Z" style="fill: rgb(255, 255, 255); paint-order: fill; stroke: rgb(0, 0, 0);"/>
              </symbol>

              <symbol id="num2" viewBox="0 0 32 32">
                <path d="M 15.342 29.543 L 15.342 26.659 L 19.43 23.215 Q 20.914 21.955 21.67 21.073 Q 22.426 20.191 22.692 19.491 Q 22.958 18.791 22.958 18.147 Q 22.958 16.747 22.132 16.061 Q 21.306 15.375 19.766 15.375 Q 18.814 15.375 17.946 15.627 Q 17.078 15.879 16.322 16.355 L 15.006 12.911 Q 16.21 12.155 17.764 11.819 Q 19.318 11.483 20.774 11.483 Q 23.378 11.483 24.974 12.337 Q 26.57 13.191 27.298 14.591 Q 28.026 15.991 28.026 17.643 Q 28.026 19.099 27.368 20.513 Q 26.71 21.927 25.03 23.355 L 22.146 25.819 L 22.146 25.931 L 28.166 25.931 L 28.166 29.543 Z" style="fill: rgb(255, 255, 255); paint-order: fill; stroke: rgb(0, 0, 0);"/>
              </symbol>

              <symbol id="num3" viewBox="0 0 32 32">
                <path d="M 21.558 29.963 Q 19.766 29.963 18.016 29.515 Q 16.266 29.067 15.006 28.031 L 16.742 24.923 Q 17.47 25.483 18.814 25.833 Q 20.158 26.183 21.39 26.183 Q 23.014 26.183 23.686 25.623 Q 24.358 25.063 24.358 24.223 Q 24.358 23.411 23.742 22.865 Q 23.126 22.319 21.782 22.319 L 18.03 22.319 L 18.03 18.847 L 21.782 18.847 Q 23.014 18.847 23.476 18.259 Q 23.938 17.671 23.938 17.027 Q 23.938 16.131 23.238 15.753 Q 22.538 15.375 21.194 15.375 Q 20.102 15.375 18.926 15.599 Q 17.75 15.823 17.05 16.243 L 15.818 12.911 Q 16.882 12.267 18.534 11.875 Q 20.186 11.483 21.866 11.483 Q 23.938 11.483 25.52 12.099 Q 27.102 12.715 27.998 13.835 Q 28.894 14.955 28.894 16.495 Q 28.894 17.755 28.306 18.833 Q 27.718 19.911 26.738 20.387 Q 28.194 21.087 28.796 22.179 Q 29.398 23.271 29.398 24.503 Q 29.398 26.155 28.418 27.387 Q 27.438 28.619 25.674 29.291 Q 23.91 29.963 21.558 29.963 Z" style="fill: rgb(255, 255, 255); paint-order: fill; stroke: rgb(0, 0, 0);"/>
              </symbol>

              <symbol id="num4" viewBox="0 0 32 32">
                <path d="M 23.294 29.543 L 23.294 26.491 L 16.21 26.491 L 15.006 23.187 L 22.706 11.763 L 27.774 11.763 L 27.774 22.991 L 30.126 22.991 L 30.126 26.491 L 27.774 26.491 L 27.774 29.543 Z M 19.458 22.991 L 23.294 22.991 L 23.294 17.111 L 23.182 17.111 Z" style="fill: rgb(255, 255, 255); paint-order: fill; stroke: rgb(0, 0, 0);"/>
              </symbol>

              <symbol id="num5" viewBox="0 0 32 32">
                <path d="M 22.118 29.963 Q 19.71 29.963 18.058 29.277 Q 16.406 28.591 15.566 27.751 L 17.582 24.615 Q 18.366 25.343 19.332 25.763 Q 20.298 26.183 21.642 26.183 Q 24.722 26.183 24.722 23.943 Q 24.722 22.851 23.966 22.249 Q 23.21 21.647 21.838 21.647 Q 21.138 21.647 20.452 21.815 Q 19.766 21.983 19.206 22.291 L 16.518 20.191 L 16.518 11.763 L 28.166 11.763 L 28.166 15.263 L 20.578 15.263 L 20.578 18.315 Q 21.11 18.175 21.796 18.063 Q 22.482 17.951 23.322 17.951 Q 26.458 17.951 27.97 19.519 Q 29.482 21.087 29.482 23.635 Q 29.482 25.427 28.698 26.869 Q 27.914 28.311 26.29 29.137 Q 24.666 29.963 22.118 29.963 Z" style="fill: rgb(255, 255, 255); paint-order: fill; stroke: rgb(0, 0, 0);"/>
              </symbol>

              <symbol id="num6" viewBox="0 0 32 32">
                <path d="M 22.93 29.963 Q 19.29 29.963 17.428 27.793 Q 15.566 25.623 15.566 21.059 Q 15.566 16.355 17.498 13.905 Q 19.43 11.455 23.042 11.455 Q 24.778 11.455 26.248 11.917 Q 27.718 12.379 28.558 12.883 L 27.494 16.047 Q 26.738 15.711 25.912 15.473 Q 25.086 15.235 24.078 15.235 Q 22.258 15.235 21.278 16.201 Q 20.298 17.167 20.298 19.239 Q 20.858 18.819 21.824 18.511 Q 22.79 18.203 23.994 18.203 Q 26.542 18.203 27.914 19.687 Q 29.286 21.171 29.286 23.915 Q 29.286 25.595 28.684 26.967 Q 28.082 28.339 26.696 29.151 Q 25.31 29.963 22.93 29.963 Z M 22.622 26.183 Q 23.602 26.183 24.064 25.567 Q 24.526 24.951 24.526 23.999 Q 24.526 22.991 24.036 22.417 Q 23.546 21.843 22.65 21.843 Q 22.118 21.843 21.502 22.067 Q 20.886 22.291 20.522 22.683 L 20.522 23.663 Q 20.522 25.035 21.138 25.609 Q 21.754 26.183 22.622 26.183 Z" style="fill: rgb(255, 255, 255); paint-order: fill; stroke: rgb(0, 0, 0);"/>
              </symbol>

              <symbol id="num7" viewBox="0 0 32 32">
                <path d="M 16.77 29.543 L 23.294 15.375 L 15.006 15.375 L 15.006 11.763 L 27.662 11.763 L 27.662 15.515 L 21.558 29.543 Z" style="fill: rgb(255, 255, 255); paint-order: fill; stroke: rgb(0, 0, 0);"/>
              </symbol>

              <symbol id="num8" viewBox="0 0 32 32">
                <path d="M 22.622 29.963 Q 19.094 29.963 17.33 28.451 Q 15.566 26.939 15.566 24.587 Q 15.566 22.991 16.28 21.955 Q 16.994 20.919 18.114 20.387 Q 17.358 19.827 16.798 18.903 Q 16.238 17.979 16.238 16.635 Q 16.238 15.123 17.022 13.961 Q 17.806 12.799 19.262 12.141 Q 20.718 11.483 22.734 11.483 Q 25.758 11.483 27.438 12.939 Q 29.118 14.395 29.118 16.691 Q 29.118 17.979 28.586 18.903 Q 28.054 19.827 27.158 20.387 Q 28.446 20.891 29.174 21.941 Q 29.902 22.991 29.902 24.531 Q 29.902 26.939 28.012 28.451 Q 26.122 29.963 22.622 29.963 Z M 22.51 18.875 Q 23.49 18.651 23.938 18.063 Q 24.386 17.475 24.386 16.719 Q 24.386 15.963 23.952 15.501 Q 23.518 15.039 22.678 15.039 Q 21.838 15.039 21.404 15.501 Q 20.97 15.963 20.97 16.691 Q 20.97 17.503 21.432 18.049 Q 21.894 18.595 22.51 18.875 Z M 22.734 26.323 Q 23.854 26.323 24.456 25.763 Q 25.058 25.203 25.058 24.419 Q 25.058 23.551 24.428 22.949 Q 23.798 22.347 22.958 22.067 Q 21.754 22.347 21.082 22.963 Q 20.41 23.579 20.41 24.419 Q 20.41 25.203 21.026 25.763 Q 21.642 26.323 22.734 26.323 Z" style="fill: rgb(255, 255, 255); paint-order: fill; stroke: rgb(0, 0, 0);"/>
              </symbol>

              <symbol id="num9" viewBox="0 0 32 32">
                <path d="M 21.474 29.991 Q 19.626 29.991 18.352 29.473 Q 17.078 28.955 16.238 28.367 L 17.974 25.119 Q 18.59 25.567 19.388 25.889 Q 20.186 26.211 21.222 26.211 Q 23.042 26.211 23.868 25.147 Q 24.694 24.083 24.694 22.207 Q 24.218 22.655 23.364 22.949 Q 22.51 23.243 21.306 23.243 Q 18.618 23.243 17.092 21.759 Q 15.566 20.275 15.566 17.559 Q 15.566 15.851 16.238 14.479 Q 16.91 13.107 18.352 12.295 Q 19.794 11.483 22.118 11.483 Q 25.702 11.483 27.564 13.611 Q 29.426 15.739 29.426 20.163 Q 29.426 24.811 27.368 27.401 Q 25.31 29.991 21.474 29.991 Z M 22.51 19.603 Q 23.714 19.603 24.47 18.763 L 24.47 17.783 Q 24.47 16.411 23.854 15.837 Q 23.238 15.263 22.37 15.263 Q 21.418 15.263 20.872 15.865 Q 20.326 16.467 20.326 17.447 Q 20.326 18.455 20.9 19.029 Q 21.474 19.603 22.51 19.603 Z" style="fill: rgb(255, 255, 255); paint-order: fill; stroke: rgb(0, 0, 0);"/>
              </symbol>

              <symbol id="numinf" viewBox="0 0 32 32">
                <path d="M 16.005 27.471 Q 14.605 27.471 13.345 26.911 Q 12.085 26.351 11.301 25.119 Q 10.517 23.887 10.517 21.871 Q 10.517 19.855 11.287 18.623 Q 12.057 17.391 13.303 16.831 Q 14.549 16.271 15.921 16.271 Q 17.461 16.271 18.637 16.831 Q 19.813 17.391 20.345 18.147 Q 20.905 17.391 22.067 16.831 Q 23.229 16.271 24.769 16.271 Q 26.169 16.271 27.401 16.831 Q 28.633 17.391 29.403 18.623 Q 30.173 19.855 30.173 21.871 Q 30.173 23.887 29.389 25.119 Q 28.605 26.351 27.359 26.911 Q 26.113 27.471 24.685 27.471 Q 23.229 27.471 22.067 26.939 Q 20.905 26.407 20.345 25.511 Q 19.813 26.407 18.651 26.939 Q 17.489 27.471 16.005 27.471 Z M 16.257 23.579 Q 17.013 23.579 17.615 23.131 Q 18.217 22.683 18.413 21.703 Q 18.133 21.087 17.587 20.625 Q 17.041 20.163 16.229 20.163 Q 15.501 20.163 15.039 20.597 Q 14.577 21.031 14.577 21.871 Q 14.577 22.711 15.039 23.145 Q 15.501 23.579 16.257 23.579 Z M 24.433 23.579 Q 25.189 23.579 25.651 23.145 Q 26.113 22.711 26.113 21.871 Q 26.113 21.031 25.651 20.597 Q 25.189 20.163 24.461 20.163 Q 23.649 20.163 23.103 20.625 Q 22.557 21.087 22.277 21.703 Q 22.501 22.683 23.089 23.131 Q 23.677 23.579 24.433 23.579 Z" style="fill: rgb(255, 255, 255); paint-order: fill; stroke: rgb(0, 0, 0);"/>
              </symbol>

              <symbol id="lb_white_wrapped" viewBox="0 0 32 32">
                <path d="M 20.173 6.827 L 31.609 6.827 L 25.89 16.122 L 31.609 25.418 L 20.173 25.418 L 15.939 32.297 L 11.707 25.418 L 0.269 25.418 L 5.988 16.122 L 0.269 6.827 L 11.707 6.827 L 15.939 -0.053 L 20.173 6.827 Z" fill="white" stroke-width="1.363" stroke="transparent"></path>
                <path d="M 19.844 7.794 L 16 1.546 L 12.156 7.794 C 12.156 7.794 2.063 7.794 2.063 7.794 L 7.11 15.997 C 7.11 15.997 2.063 24.201 2.063 24.201 L 12.156 24.201 C 12.156 24.201 16 30.45 16 30.45 L 19.844 24.201 C 19.844 24.201 29.939 24.201 29.939 24.201 L 24.891 15.997 C 24.891 15.997 29.939 7.794 29.939 7.794 L 19.844 7.794 Z M 18.655 22.132 L 16 17.953 L 13.347 22.132 L 16 26.447 L 18.655 22.132 Z M 10.88 9.891 L 14.759 15.997 L 10.88 22.104 L 5.816 22.104 C 5.816 22.104 9.572 15.997 9.572 15.997 L 5.816 9.891 L 10.88 9.891 Z M 21.122 9.891 L 26.185 9.891 C 26.185 9.891 22.428 15.997 22.428 15.997 L 26.185 22.104 L 21.122 22.104 L 17.243 15.997 L 21.122 9.891 Z M 13.347 9.863 L 16 5.548 L 18.655 9.863 L 16 14.041 L 13.347 9.863 Z" stroke-width="1.049" fill="black" stroke="transparent" fill-rule="evenodd"></path>
              </symbol>
            </defs>
        </svg>
    }
}

#[component]
pub fn IconWhite() -> impl IntoView {
    view! {
      <svg viewBox="0 0 32 32" width="32" height="32">
        <g class="icon_white" transform="matrix(1, 0, 0, 1, -21.594004, 46.167001)">
          <ellipse style="stroke-width: 1; stroke: rgb(255, 255, 255);" cx="37.594" cy="-30.167" rx="11.995" ry="11.995"></ellipse>
          <path d="M 37.594 -20.207 L 33.534 -22.154 L 31.838 -36.905 L 37.594 -40.128 L 43.54 -36.799 L 41.868 -22.256 L 37.594 -20.207 Z M 30.039 -23.829 L 28.944 -35.285 L 30.302 -36.045 L 31.624 -23.069 L 30.039 -23.829 Z M 46.243 -35.285 L 45.148 -23.829 L 43.776 -23.171 L 45.077 -35.938 L 46.243 -35.285 Z" style="fill: url(#gradient-1); stroke: rgb(254, 255, 252); stroke-width: 0.2px;"></path>
        </g>
      </svg>
    }
}

#[component]
pub fn IconBlue() -> impl IntoView {
    view! {
      <svg viewBox="0 0 32 32" width="32" height="32">
        <g class="icon_blue" transform="matrix(1, 0, 0, 1, -51.335995, 46.160001)">
          <ellipse style="stroke-width: 1; stroke: rgb(255, 255, 255);" cx="67.336" cy="-30.16" rx="11.995" ry="11.995"></ellipse>
          <path d="M 77.115 -30.185 C 77.115 -26.274 74.819 -22.899 71.501 -21.335 L 70.135 -27.115 C 70.94 -27.865 71.443 -28.935 71.444 -30.122 C 71.443 -32.045 70.122 -33.661 68.337 -34.108 L 68.336 -39.913 C 73.267 -39.413 77.115 -35.248 77.115 -30.185 Z M 67.336 -20.406 C 66.456 -20.406 65.602 -20.522 64.791 -20.741 L 66.084 -26.208 C 66.479 -26.082 66.899 -26.014 67.336 -26.014 C 67.773 -26.013 68.193 -26.081 68.587 -26.208 L 69.881 -20.74 C 69.069 -20.522 68.216 -20.406 67.336 -20.406 Z M 57.557 -30.185 C 57.557 -35.248 61.405 -39.413 66.336 -39.914 L 66.336 -34.108 C 64.55 -33.661 63.228 -32.046 63.228 -30.122 C 63.228 -28.935 63.732 -27.865 64.537 -27.115 L 63.17 -21.335 C 59.853 -22.9 57.557 -26.274 57.557 -30.185 Z" style="fill: url(#gradient-0); stroke: rgb(19, 235, 247); stroke-width: 0.2px;"></path>
        </g>
      </svg>
    }
}

#[component]
pub fn IconBlack() -> impl IntoView {
    view! {
      <svg viewBox="0 0 32 32" width="32" height="32">
        <g class="icon_black" transform="matrix(1, 0, 0, 1, -112.031999, 46.393002)">
          <ellipse style="stroke-width: 1; stroke: rgb(255, 255, 255);" cx="128.032" cy="-30.393" rx="11.995" ry="11.995"></ellipse>
          <path d="M 128.702 -41.112 L 132.287 -34.762 L 138.775 -31.032 L 128.702 -31.032 L 128.702 -41.112 Z M 127.34 -31.042 L 117.267 -31.042 L 123.755 -34.762 L 127.34 -41.102 L 127.34 -31.042 Z M 128.724 -29.752 L 138.797 -29.752 L 132.309 -26.022 L 128.724 -19.672 L 128.724 -29.752 Z M 127.362 -19.682 L 123.777 -26.022 L 117.289 -29.742 L 127.362 -29.742 L 127.362 -19.682 Z" style="stroke-width: 0.2; fill: url(#gradient-6); stroke: url(#gradient-7);"></path>
        </g>
      </svg>
    }
}

#[component]
pub fn IconRed() -> impl IntoView {
    view! {
      <svg viewBox="0 0 32 32" width="32" height="32">
        <g transform="matrix(1, 0, 0, 1, 5.113, 46.505002)" class="icon_red">
          <g>
            <ellipse style="stroke: rgb(255, 255, 255);" cx="10.887" cy="-30.505" rx="11.995" ry="11.995"></ellipse>
            <g transform="matrix(1, 0, 0, 1, 45.403001, -43.773873)">
              <path d="M -35.211 2.922 L -35.211 23.677 L -44.49 9.792 L -37.69 11.634 Z M -30.921 11.634 L -24.125 9.792 L -33.4 23.677 L -33.4 2.922 Z" style="fill: url(#gradient-2); stroke-width: 0.2px; stroke: rgb(252, 224, 224);"></path>
            </g>
          </g>
        </g>
      </svg>
    }
}

#[component]
pub fn IconGreen() -> impl IntoView {
    view! {
      <svg viewBox="0 0 32 32" width="32" height="32">
        <g class="icon_green" transform="matrix(1, 0, 0, 1, -80.701997, 45.929002)">
          <ellipse style="stroke-width: 1; stroke: rgb(255, 255, 255);" cx="96.702" cy="-29.929" rx="11.995" ry="11.995"></ellipse>
          <path d="M 91.933 -37.634 L 93.657 -38.704 L 93.657 -22.806 L 88.247 -22.806 L 91.933 -37.634 Z M 96.701 -40.593 L 98.26 -39.626 L 98.26 -22.805 L 95.142 -22.806 L 95.142 -39.625 L 96.701 -40.593 Z M 101.471 -37.633 L 105.157 -22.805 L 99.745 -22.805 L 99.745 -38.704 L 101.471 -37.633 Z" style="fill: url(#gradient-3); stroke: rgb(186, 218, 85); stroke-width: 0.2px;"></path>
        </g>
      </svg>
    }
}

#[component]
pub fn IconColorless() -> impl IntoView {
    view! {
      <svg viewBox="0 0 32 32" width="32" height="32">

        <g>
          <ellipse style="stroke-width: 1; stroke: rgb(255, 255, 255);" cx="16" cy="16" rx="11.995" ry="11.995"></ellipse>
          <rect x="0.755" y="46.135" width="12.816" height="12.816" style="stroke: url(#gradient-8); fill: url(#gradient-9); stroke-width: 0.8; transform-box: fill-box; transform-origin: 50% 50%;" transform="matrix(0.707107, 0.707107, -0.707107, 0.707107, 8.836999, -36.543025)"></rect>
        </g>

        <g class="icon_colorless" transform="matrix(1, 0, 0, 1, -75.766999, -8.232001)">
          <ellipse style="stroke-width: 1; stroke: rgb(255, 255, 255);" cx="91.767" cy="24.232" rx="11.995" ry="11.995"></ellipse>
          <rect x="0.755" y="46.135" width="12.816" height="12.816" style="stroke: url(#gradient-8); fill: url(#gradient-9); stroke-width: 0.8px; transform-box: fill-box; transform-origin: 50% 50%;" transform="matrix(0.707107, 0.707107, -0.707107, 0.707107, 84.603999, -28.311009)"></rect>
        </g>
      </svg>
    }
}

#[component]
pub fn IconCoin() -> impl IntoView {
    view! {
      <svg viewBox="0 0 32 32" width="32" height="32">
          <defs>
          </defs>
          <circle style="fill: rgb(255, 200, 2); stroke: rgb(255, 228, 191);" cx="16" cy="16" r="14.525"></circle>
          <circle style="stroke-width: 1; stroke: #ff9e21; fill: transparent;" cx="16" cy="16" r="12.686"></circle>
      </svg>
    }
}

#[component]
pub fn IconPripara() -> impl IntoView {
    view! {
        <svg viewBox="0 0 32 32" wodth="32" height="32">
          <defs>
            <filter id="drop-shadow-filter-0" color-interpolation-filters="sRGB" x="-50%" y="-50%" width="200%" height="200%">
              <feGaussianBlur in="SourceAlpha" stdDeviation="2"></feGaussianBlur>
              <feOffset dx="0" dy="0"></feOffset>
              <feComponentTransfer result="offsetblur">
                <feFuncA id="spread-ctrl" type="linear" slope="1"></feFuncA>
              </feComponentTransfer>
              <feFlood flood-color="#ffffff4d"></feFlood>
              <feComposite in2="offsetblur" operator="in"></feComposite>
              <feMerge>
                <feMergeNode></feMergeNode>
                <feMergeNode in="SourceGraphic"></feMergeNode>
              </feMerge>
            </filter>
            <filter id="gaussian-blur-filter-0" color-interpolation-filters="sRGB" x="-50%" y="-50%" width="200%" height="200%">
              <feGaussianBlur stdDeviation="0.8 0.8" edgeMode="none"></feGaussianBlur>
            </filter>
          </defs>
          <path style="stroke: rgb(253, 12, 190); fill: rgb(247, 103, 194); filter: url(#drop-shadow-filter-0); transform-box: fill-box; transform-origin: 50% 50%;" d="M 16.284 28.485 L 16 28.627 L 15.716 28.485 C 2.113 19.127 0.502 11.298 3.135 6.981 C 5.765 2.663 12.639 1.86 16 6.553 C 19.363 1.86 26.235 2.663 28.865 6.981 C 31.498 11.298 29.887 19.127 16.284 28.485 Z" transform="matrix(0.994522, 0.104528, -0.104528, 0.994522, 0, 0.000001)"></path>
          <path style="stroke-width: 1; fill: rgb(255, 164, 221); filter: url(#gaussian-blur-filter-0); transform-box: fill-box; transform-origin: 50% 50%;" d="M 7.61 15.228 C 6.313 12.755 6.472 11.213 7.385 9.924 C 8.038 8.569 9.174 8.063 10.772 8.016 C 12.375 7.867 13.561 8.205 14.423 9.72 L 16.071 12.152 L 16.624 11.337 L 17.732 9.701 C 18.594 8.186 19.789 7.868 21.392 8.017 C 22.987 8.063 24.151 8.649 24.807 10.006 C 25.719 11.292 25.824 12.764 24.528 15.239 C 23.239 17.958 20.811 20.711 16.071 24.072 C 11.332 20.711 8.898 17.946 7.61 15.228 Z" transform="matrix(0.994522, 0.104528, -0.104528, 0.994522, 0, 0)"></path>
        </svg>
    }
}

#[component]
pub fn IconNijisanji() -> impl IntoView {
    view! {
        <svg viewBox="0 0 32 32" width="32" height="32">
          <defs>
            <radialGradient gradientUnits="userSpaceOnUse" cx="18.611" cy="50.113" r="9.094" id="gradient-0" gradientTransform="matrix(0.901401, -0.901401, 0.911898, 0.911898, -44.795008, -14.39729)">
              <stop offset="0" style="stop-color: rgb(222, 0, 255);"></stop>
              <stop offset="0.201" style="stop-color: rgb(7, 0, 255);"></stop>
              <stop offset="0.349" style="stop-color: rgb(0, 225, 255);"></stop>
              <stop offset="0.488" style="stop-color: rgb(26, 255, 3);"></stop>
              <stop offset="0.66" style="stop-color: rgb(255, 227, 0);"></stop>
              <stop offset="0.817" style="stop-color: rgb(255, 145, 0);"></stop>
              <stop offset="1" style="stop-color: rgb(255, 0, 0);"></stop>
            </radialGradient>
          </defs>
          <path style="fill: rgb(0, 50, 113);" d="M 27.328 16.448 C 27.328 25.69 17.322 31.468 9.317 26.846 C 8.999 26.662 8.692 26.467 8.399 26.262 L 3.458 28.89 L 5.534 23.436 C 1.62 17.964 2.881 9.766 9.317 6.049 C 11.142 4.995 13.213 4.441 15.321 4.441 L 15.321 16.448 L 27.328 16.448 Z"></path>
          <path d="M 17.574 14.444 L 17.574 2.441 C 24.009 2.64 29.164 7.918 29.164 14.402 C 29.164 14.415 29.164 14.429 29.164 14.444 L 17.574 14.444 Z" style="fill: url(#gradient-0);"></path>
          <path style="fill: rgb(255, 255, 255); transform-box: fill-box; transform-origin: 50% 50%;" d="M -16.483 37.285 A 3.289 3.289 0 1 1 -16.483 40.574 A 1.91 1.91 0 1 0 -16.483 37.285 Z" transform="matrix(-0.707107, 0.707107, -0.707107, -0.707107, 23.308246, -25.833031)"></path>
        </svg>
    }
}
#[component]
pub fn IconDissona() -> impl IntoView {
    view! {
      <svg viewBox="0 0 32 32" width="32" height="32">
        <path d="M 18.435 8 L 18.435 2.931 L 13.565 2.931 L 13.565 8 L 8.198 8 L 8.198 13.565 L 2.931 13.565 L 2.931 18.435 L 8.198 18.435 L 8.198 23.95 L 13.565 23.95 L 13.565 29.069 L 18.435 29.069 L 18.435 23.95 L 24.149 23.95 L 24.149 18.435 L 29.069 18.435 L 29.069 13.565 L 24.149 13.565 L 24.149 8 L 18.435 8 Z" style="stroke: rgb(3, 0, 0); stroke-width: 5.552;"></path>
        <path d="M 18.435 8 L 18.435 2.931 L 13.565 2.931 L 13.565 8 L 8.198 8 L 8.198 13.565 L 2.931 13.565 L 2.931 18.435 L 8.198 18.435 L 8.198 23.95 L 13.565 23.95 L 13.565 29.069 L 18.435 29.069 L 18.435 23.95 L 24.149 23.95 L 24.149 18.435 L 29.069 18.435 L 29.069 13.565 L 24.149 13.565 L 24.149 8 L 18.435 8 Z" style="stroke: rgb(255, 164, 164); stroke-width: 2.06; stroke-linejoin: miter; stroke-miterlimit: 1;"></path>
        <path d="M 19.758 12.477 C 20.532 12.157 21.275 12.101 21.867 12.289 C 21.446 13.243 20.658 14.166 19.657 14.769 C 18.699 15.347 17.742 15.518 16.994 15.322 C 16.978 15.33 16.961 15.337 16.944 15.345 C 16.94 15.332 16.936 15.318 16.932 15.304 C 16.918 15.3 16.905 15.297 16.891 15.292 C 16.899 15.275 16.906 15.258 16.913 15.241 C 16.718 14.494 16.888 13.537 17.466 12.579 C 18.07 11.578 18.993 10.79 19.947 10.368 C 20.135 10.961 20.079 11.704 19.758 12.477 Z" style="fill: rgb(255, 173, 173); stroke-width: 0.324; transform-origin: 82.914px 293.35px;"></path>
        <path d="M 12.242 12.477 C 11.467 12.156 10.725 12.101 10.132 12.289 C 10.554 13.243 11.341 14.165 12.343 14.769 C 13.3 15.347 14.258 15.518 15.004 15.322 C 15.022 15.33 15.039 15.337 15.056 15.345 C 15.06 15.332 15.064 15.318 15.068 15.304 C 15.082 15.3 15.095 15.297 15.109 15.292 C 15.101 15.275 15.094 15.258 15.085 15.241 C 15.282 14.494 15.111 13.536 14.534 12.579 C 13.928 11.577 13.007 10.79 12.053 10.369 C 11.865 10.962 11.92 11.704 12.242 12.477 Z" style="fill: rgb(255, 173, 173); stroke-width: 0.324; transform-origin: 76.753px 293.35px;"></path>
        <path d="M 19.758 19.524 C 20.532 19.844 21.275 19.9 21.867 19.711 C 21.446 18.758 20.658 17.835 19.657 17.232 C 18.699 16.654 17.742 16.483 16.994 16.679 C 16.978 16.671 16.961 16.663 16.944 16.656 C 16.94 16.669 16.936 16.682 16.932 16.697 C 16.918 16.7 16.905 16.705 16.891 16.709 C 16.899 16.726 16.906 16.743 16.913 16.76 C 16.718 17.506 16.888 18.464 17.466 19.422 C 18.07 20.424 18.993 21.211 19.947 21.632 C 20.135 21.039 20.079 20.297 19.758 19.524 Z" style="fill: rgb(255, 173, 173); stroke-width: 0.324; transform-origin: 82.914px 300.048px;"></path>
        <path d="M 12.241 19.524 C 11.467 19.845 10.725 19.9 10.132 19.711 C 10.554 18.758 11.341 17.836 12.343 17.232 C 13.3 16.654 14.258 16.483 15.004 16.679 C 15.022 16.671 15.039 16.663 15.056 16.656 C 15.06 16.67 15.064 16.682 15.068 16.697 C 15.082 16.7 15.095 16.705 15.109 16.709 C 15.101 16.726 15.094 16.743 15.085 16.76 C 15.282 17.506 15.111 18.465 14.534 19.422 C 13.928 20.424 13.007 21.211 12.053 21.632 C 11.865 21.039 11.92 20.298 12.241 19.524 Z" style="fill: rgb(255, 173, 173); stroke-width: 0.324; transform-origin: 76.753px 300.048px;"></path>
      </svg>
    }
}
#[component]
pub fn IconDenonbu() -> impl IntoView {
    view! {
      <svg viewBox="0 0 32 32" width="32" height="32">
        <rect x="-4.039" y="-3.323" width="40.078" height="38.647" style=""></rect>
        <path style="fill: rgb(255, 255, 255);" d="M 2.469 14.531 L 1.7 13.418 L 7.728 13.418 L 4.813 8.889 L 2.124 8.889 L 1.275 7.89 L 14.062 7.89 L 11.911 5.011 L 12.871 4.013 L 19.129 4.013 L 20.089 5.011 L 17.938 7.89 L 30.725 7.89 L 29.876 8.889 L 27.187 8.889 L 24.272 13.418 L 30.3 13.418 L 29.531 14.531 L 2.469 14.531 Z"></path>
        <path d="M 3.768 15.028 L 28.097 15.028 L 29.018 15.911 L 23.788 24.971 L 8.077 24.971 L 2.846 15.911 L 3.768 15.028 Z" style="stroke-width: 1; fill: rgb(255, 255, 255);"></path>
        <path d="M 15.991 12.723 L 13.486 8.701 L 18.496 8.701 L 15.991 12.723 Z" style=""></path>
        <path d="M 20.072 16.729 L 18.942 18.546 L 17.499 18.546 L 18.514 17.343 L 17.711 17.343 L 15.634 18.689 L 16.059 17.839 L 12.613 17.839 L 11.929 16.753 L 12.605 16.075 L 19.62 16.075 L 20.072 16.729 Z" style=""></path>
        <polygon style="" points="14.911 18.618 13.137 18.618 16.185 23.74 18.546 19.821 16.208 19.821 17.106 18.57 14.415 20.034"></polygon>
      </svg>
    }
}
#[component]
pub fn IconBlueArchive() -> impl IntoView {
    view! {
      <svg viewBox="0 0 32 32" width="32" height="32">
        <defs>
          <pattern id="pattern-0" viewBox="0 0 79.685 79.685" patternUnits="userSpaceOnUse" preserveAspectRatio="none" width="100" ight="100">
            <rect x="-59.734" y="87.839" width="79.685" height="79.685" style="fill: rgb(0, 208, 255);" transform="matrix(1, 0, 0, 1, 59.73400115966797, -87.83900451660156)"></rect>
            </pattern>
          </defs>
          <rect x="0.024" y="0.137" width="31.951" height="31.727" style=""></rect>
          <path d="M 33.544 15.056 C 33.544 15.65 33.402 16.227 33.133 16.782 C 33.27 16.391 33.342 15.99 33.342 15.581 C 33.342 11.635 26.673 8.436 18.446 8.436 C 10.219 8.436 3.55 11.635 3.55 15.581 C 3.55 18.075 6.854 19.065 10.891 20.343 C 5.564 19.607 2.54 17.827 2.54 15.056 C 2.54 10.91 9.443 7.397 18.005 7.397 C 26.566 7.397 33.544 10.91 33.544 15.056 Z" style="fill: rgb(255, 255, 255); transform-box: fill-box; transform-origin: 50% 50%;" transform="matrix(0.866025, 0.5, -0.5, 0.866025, 0.000001, 0.000002)"></path>
          <path d="M 12.45 35.521 L 13.569 13.331 L -2.002 13.641 L 13.682 12.279 L 15.437 -2.103 L 14.886 12.221 L 30.817 12.027 L 14.8 13.27 L 12.45 35.521 Z" style="stroke-width: 1; fill: rgb(154, 228, 255); transform-box: fill-box; transform-origin: 50% 50%;" transform="matrix(0.866025, 0.5, -0.5, 0.866025, 0.000002, -0.000002)"></path>
      </svg>
    }
}

#[component]
pub fn IconNext() -> impl IntoView {
    view! {
      <svg viewBox="0 0 32 32" width="32" height="32">
          <path d="M 27.093 16.001 L 4.906 27.094 L 4.906 4.907 L 27.093 16.001 Z" style="paint-order: stroke; fill: rgb(255, 255, 255); stroke-width: 0px; transform-origin: 16px 16.001px;"></path>
      </svg>
    }
}

#[component]
pub fn IconPrevious() -> impl IntoView {
    view! {
      <svg viewBox="0 0 32 32" width="32" height="32">
          <path d="M 4.906 16.001 L 27.093 27.094 L 27.093 4.907 L 4.906 16.001 Z" style="paint-order: stroke; fill: rgb(255, 255, 255); transform-origin: 16px 16px; stroke-width: 0px;"></path>
      </svg>
    }
}

#[component]
pub fn ColorIconWithNum(code: String) -> impl IntoView {
    // Parse the code to extract color and number
    let (color_char, number_str) = if code.len() >= 2 {
        let mut chars = code.chars();
        let color = chars.next().unwrap_or('w');
        let number: String = chars.collect();
        (color, number)
    } else {
        ('w', "0".to_string())
    };

    // Get the appropriate color icon component
    let color_icon = match color_char.to_ascii_lowercase() {
        'r' => view! { <IconRed /> }.into_any(),
        'u' => view! { <IconBlue /> }.into_any(),
        'g' => view! { <IconGreen /> }.into_any(),
        'w' => view! { <IconWhite /> }.into_any(),
        'k' => view! { <IconBlack /> }.into_any(),
        'l' => view! { <IconColorless /> }.into_any(),
        'c' => view! { <IconCoin /> }.into_any(),
        _ => view! { <IconWhite /> }.into_any(), // Default to white if unknown
    };

    // Get the number symbol ID
    let number_symbol_id = match number_str.as_str() {
        "0" => "#num0",
        "1" => "#num1",
        "2" => "#num2",
        "3" => "#num3",
        "4" => "#num4",
        "5" => "#num5",
        "6" => "#num6",
        "7" => "#num7",
        "8" => "#num8",
        "9" => "#num9",
        "inf" => "#numinf",
        _ => "#num0", // Default to 0 if unknown
    };

    view! {
        <div style="position: relative; display: inline-block; width: 32px; height: 32px;">
            // Color icon as background
            <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;">
                {color_icon}
            </div>
            // Number overlay
            <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none;">
                <svg viewBox="0 0 32 32" width="32" height="32" style="width: 100%; height: 100%;">
                    <use href={number_symbol_id} />
                </svg>
            </div>
        </div>
    }
}

#[component]
pub fn ColorIconsWithNum(code: String) -> impl IntoView {
    // Parse the code to extract multiple color/number pairs
    let parsed_icons = parse_multiple_color_codes(&code);

    view! {
        <div style="display: inline-flex; align-items: center; gap: 2px;">
            {parsed_icons.into_iter().map(|icon_code| {
                view! {
                    <ColorIconWithNum code={icon_code} />
                }
            }).collect::<Vec<_>>()}
        </div>
    }
}

// Helper function to parse multiple color/number codes like "w1r3b2"
fn parse_multiple_color_codes(code: &str) -> Vec<String> {
    let mut result = Vec::new();
    let mut current_color = None;
    let mut current_number = String::new();

    for ch in code.chars() {
        if "rgwkulc".contains(ch.to_ascii_lowercase()) {
            // If we have a previous color/number pair, save it
            if let Some(color) = current_color {
                if current_number.is_empty() {
                    current_number = "0".to_string();
                }
                result.push(format!("{}{}", color, current_number));
            }
            // Start new pair
            current_color = Some(ch.to_ascii_lowercase());
            current_number.clear();
        } else if ch.is_ascii_digit() {
            current_number.push(ch);
        } else if current_number.is_empty() && ch == 'i' {
            // Check for "inf" pattern
            current_number.push(ch);
        } else if current_number == "i" && ch == 'n' {
            current_number.push(ch);
        } else if current_number == "in" && ch == 'f' {
            current_number.push(ch);
        }
    }

    // Don't forget the last pair
    if let Some(color) = current_color {
        if current_number.is_empty() {
            current_number = "0".to_string();
        }
        result.push(format!("{}{}", color, current_number));
    }

    result
}
