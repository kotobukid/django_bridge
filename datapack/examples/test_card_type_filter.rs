use datapack::{
    fetch_by_colors_features_and_card_types_native, get_all_cards, CardExport, CardType,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Unknown card type filter results: {}",
        unknown_results.len()
    );
    ...
    for card_type in &all_types {
        ...
        let results = fetch_by_colors_features_and_card_types_native(&cards, 0, &[], &filter);
        println!(
            "{} ({}): {} cards",
    ...
    for card_type in &all_types {
        let filter = vec![card_type.clone()];
        let results = fetch_by_colors_features_and_card_types_native(&mock_cards, 0, &[], &filter);
        println!(
            "Mock {} ({}): {} cards",
            card_type.display_name(),
            card_type.to_u8(),
