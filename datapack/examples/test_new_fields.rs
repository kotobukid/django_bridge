...
fn main() {
    ...
    let cards_with_level: Vec<_> = cards.iter()
        .filter(|c| !c.level().is_empty())
        .take(3)
        .collect();
    
    let cards_with_power: Vec<_> = cards.iter()
        .filter(|c| !c.power().is_empty())
        .take(3)
        .collect();
    
    let cards_with_limit: Vec<_> = cards.iter()
    ...
    for card in cards_with_level {
        println!("Name: {}, Level: {}, Type: {}", card.name(), card.level(), card.card_type());
    }
    
    println!("\n=== Cards with Power ===");
    for card in cards_with_power {
        println!("Name: {}, Power: {}, Type: {}", card.name(), card.power(), card.card_type());
    }
    
    println!("\n=== Cards with Limit ===");
    for card in cards_with_limit {
        println!("Name: {}, Limit: {}, Type: {}", card.name(), card.limit(), card.card_type());
    }
